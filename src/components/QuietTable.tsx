...
export function QuietTable({ locale }: { locale: "ro" | "en" }) {
  ...
  return (
    <div className={`quiet-table ${mode}`}>
      ...
      <div className="table-wood"><span className="root-line r1" /><span className="root-line r2" /><span className="root-line r3" /></div>
      <div className="table-controls" aria-label={locale === "ro" ? "Alege semnalul mesei" : "Choose table signal"}>
        <button className={mode === "quiet" ? "active" : ""} onClick={() => setMode("quiet")}>QUIET COMPANY</button>
        <button className={mode === "open" ? "active" : ""} onClick={() => setMode("open")}>OPEN TO CONVERSATION</button>
      </div>
